...
/**
 * Tests for {@link ZoneFailoverAwareServiceInstanceListSupplier}.
 *
 * @author Jiwon Jeon
 */
class ZoneFailoverAwareServiceInstanceListSupplierTests {

	private static final String METADATA_KEY_ZONE = "zone";

	private final DiscoveryClientServiceInstanceListSupplier delegate = mock(DiscoveryClientServiceInstanceListSupplier.class);
	...
	private ZoneFailoverAwareCacheDataManager cacheDataManager = mock(ZoneFailoverAwareCacheDataManager.class);
	private ZoneFailoverAwareServiceInstanceListSupplier supplier = new ZoneFailoverAwareServiceInstanceListSupplier(delegate, cacheDataManager, zoneConfig);

	@Test
	void shouldFilterInstancesByZoneWhenNoAZFailed() {
		when(cacheDataManager.getFailedInstances()).thenReturn(Collections.EMPTY_SET);
	...
	@Test
	void shouldFilterInstancesByZoneWhenAnAZFailed() {
		when(cacheDataManager.getFailedInstances()).thenReturn(new HashSet<>(List.of(first, fifth)));
		when(delegate.get()).thenReturn(Flux.just(List.of(first, second, third, fourth, fifth)));
	...
	private Map<String, String> buildZoneMetadata(String zone) {
		Map<String, String> metadata = new HashMap<>(); metadata.put(METADATA_KEY_ZONE, zone); return metadata;
	}
