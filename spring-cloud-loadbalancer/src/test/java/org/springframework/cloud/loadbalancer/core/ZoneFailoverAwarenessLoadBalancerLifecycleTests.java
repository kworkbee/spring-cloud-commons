...
class ZoneFailoverAwarenessLoadBalancerLifecycleTests {
	...
	private final ZoneFailoverAwarenessLoadBalancerLifecycle lifecycle =
			new ZoneFailoverAwarenessLoadBalancerLifecycle(cacheDataManager);

	@Test
	void shouldAnInstanceNotBeAddedIntoCache() {
		final CompletionContext completionContext =
				new CompletionContext<ResponseData, ServiceInstance, RetryableRequestContext>(
						CompletionContext.Status.SUCCESS, getRequest(), new DefaultResponse(getServiceInstance()));

		lifecycle.onComplete(completionContext);

		verify(cacheDataManager, never()).put(any(ServiceInstance.class));
	}
