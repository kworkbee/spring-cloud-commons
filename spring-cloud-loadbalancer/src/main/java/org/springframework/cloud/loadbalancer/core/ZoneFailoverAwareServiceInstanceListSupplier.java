...
public class ZoneFailoverAwareServiceInstanceListSupplier extends DelegatingServiceInstanceListSupplier {
	...
	public ZoneFailoverAwareServiceInstanceListSupplier(ServiceInstanceListSupplier delegate,
														ZoneFailoverAwareCacheDataManager cacheDataManager,
	...
	private List<ServiceInstance> filteredAndOrderedByZone(List<ServiceInstance> serviceInstances) {
		...
		final List<String> zones =
				Stream.concat(List.of(zoneConfig.getZone()).stream(), zoneConfig.getSecondaryZones().stream())
						.toList();
		final Stream instanceStream = serviceInstances.stream()
				.sorted(Comparator.comparing(instance -> zones
